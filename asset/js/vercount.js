...
!function () {
    ...
    const o = "visitorCountData",
        a = async () => {
            ...
            try {
                const o = await fetch("https://events.vercount.one/api/v2/log", {
                    method: "POST",
                    headers: {
                ...
                if (clearTimeout(n), !o.ok) throw new Error(`HTTP ${o.status}`);
                return (t => "success" === t?.status && t.data ? t.data : "error" === t?.status ? (console.warn("API error:", t.message), t.data || {
                    site_uv: 0,
                    site_pv: 0,
                    page_pv: 0
                }) : t || {
                ...
                return clearTimeout(n), "AbortError" === t.name ? console.warn("Request timeout") : console.warn("API error:", t.message), null
            }
        },
        s = () => {
            if (n) return n;
            return n = {}, ["site_pv", "page_pv", "site_uv"].forEach((t => {
                n[`busuanzi_value_${t}`] = document.getElementById(`busuanzi_value_${t}`),
                    n[`busuanzi_container_${t}`] = document.getElementById(`busuanzi_container_${t}`),
                    n[`vercount_value_${t}`] = document.getElementById(`vercount_value_${t}`),
                    n[`vercount_container_${t}`] = document.getElementById(`vercount_container_${t}`)
            })), n
        },
        r = t => {
            const e = s();
            ["site_pv", "page_pv", "site_uv"].forEach((n => {
                const o = String(t[n] || "0"),
                    a = e[`busuanzi_value_${n}`],
                    s = e[`vercount_value_${n}`];

                // 修改这里：使用 numberToImageTags 函数
                const numberHtml = numberToImageTags(o);

                a && (a.innerHTML = numberHtml), // 修改：使用 innerHTML 而不是 textContent
                    s && (s.innerHTML = numberHtml)  // 修改：使用 innerHTML 而不是 textContent
            }))
        },
        u = () => {
            const t = s();
            ["site_pv", "page_pv", "site_uv"].forEach((e => {
                const n = t[`busuanzi_container_${e}`],
                    o = t[`vercount_container_${e}`];
    ...
    }, e || "loading" !== document.readyState ? i() : (t.push(i), document.addEventListener("DOMContentLoaded", (() => {
        e = !0, t.forEach((t => t()))
    }), {
        once: !0
    }))
